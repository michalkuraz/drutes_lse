...
contains
  ...
  subroutine printtime(message, t)
    ...
    select case (int(t))
      ...
      case default
        write(unit=terminal, fmt=*) trim(message), t/31536000, " " // achar(27)//'[43m', &
             " YEARS !!!!...:)" // achar(27)//'[0m'
    end select
  ...
  ! tmp, tmp1, sim_time, start_time, end_time, time, www
  ! are taken from module globals

  real    :: now
  integer :: fileid, ierrtime

  ! Try to read system uptime (Linux); fall back to CPU_TIME otherwise
  ...
  ! Progress percentage
  tmp1 = min(100.0, time/end_time*100.0)
  ...
  select case (int(tmp))
    ...
    case default
      write(unit=terminal, fmt=*) " #time to finish =", tmp/31536000, " " // achar(27)//'[43m', &
           " YEARS !!!!...:)" // achar(27)//'[0m'
  end select
  ...
  if (www) then
    ...
    select case (nint(tmp))
      ...
      case default
        write(unit=fileid, fmt=*) " #time to finish =", tmp/31536000, " " // achar(27)//'[43m', &
             " YEARS !!!!...:)" // achar(27)//'[0m'
    end select
